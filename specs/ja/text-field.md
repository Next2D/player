...
// TextFormatを作成
const format = new next2d.text.TextFormat();
format.font = "Arial";
format.size = 24;
...
```javascript
const inputField = new next2d.text.TextField();
...
inputField.addEventListener("change", (event) => {
  console.log("入力値:", event.target.text);
});
...
```javascript
const passwordField = new next2d.text.TextField();
...
```javascript
const scoreField = new next2d.text.TextField();
