...
describe("ContextResizeUseCase.js method test", () =>
{
    it("test case", () =>
    {
        ...
        expect(context.$mainAttachmentObject.msaa).toBe(false);
        expect(context.$mainAttachmentObject.mask).toBe(false);
        expect(context.$mainAttachmentObject.texture?.resource).toBe("createTexture");
        expect(context.$mainAttachmentObject.texture?.area).toBe(100 * 200);
        expect(context.$mainAttachmentObject.texture?.width).toBe(100);
        expect(context.$mainAttachmentObject.texture?.height).toBe(200);
        expect(context.$mainAttachmentObject.color).toBe(null);
        expect(context.$mainAttachmentObject.stencil?.resource).toBe("createRenderbuffer");
        expect(context.$mainAttachmentObject.stencil?.width).toBe(100);
        expect(context.$mainAttachmentObject.stencil?.height).toBe(200);
    });
});