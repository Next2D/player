...
describe("FilterApplyGlowFilterUseCase", () =>
{
    ...
    describe("basic glow execution", () =>
    {
        it("should apply blur filter first", () =>
        {
            ...
            // Blur filter should have been applied (via mock)
            expect(config.frameBufferManager.createTemporaryAttachment).toHaveBeenCalled();
        });

        it("should create uniform buffer with color data", () =>
        {
            const sourceAttachment = createMockAttachment();
            const matrix = new Float32Array([1, 0, 0, 1, 0, 0]);
            const config = createMockConfig();
    ...
    describe("inner glow mode", () =>
    {
        it("should use base size for inner glow", () =>
        {
            ...
            // For inner glow, output should match base size
            expect(config.frameBufferManager.createTemporaryAttachment).toHaveBeenCalled();
        });

        it("should restore offset for inner glow", () =>
        {
            $offset.x = 5;
            $offset.y = 5;
