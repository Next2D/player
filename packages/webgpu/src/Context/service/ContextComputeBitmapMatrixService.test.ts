...
{
    // identity context matrix [a, b, 0, c, d, 0, tx, ty, 1]
    const identityContext = new Float32Array([1, 0, 0, 0, 1, 0, 0, 0, 1]);

    it("should compute inverse of identity matrix", () =>
    {
        const bitmapMatrix = new Float32Array([1, 0, 0, 1, 0, 0]); // identity

        const result = execute(bitmapMatrix, identityContext);
    ...
    {
        const bitmapMatrix = new Float32Array([2, 0, 0, 3, 0, 0]); // scale(2, 3)
    ...
    {
        const bitmapMatrix = new Float32Array([1, 0, 0, 1, 100, 50]); // translate(100, 50)
    ...
    it("should compute inverse of rotation matrix", () =>
    {
        ...
        const bitmapMatrix = new Float32Array([cos, sin, -sin, cos, 0, 0]);

        const result = execute(bitmapMatrix, identityContext);

        // Inverse of rotation is rotation by negative angle
        // Output is column-major: [a, b, 0, c, d, 0, tx, ty, 1]
        // Inverse rotation matrix: a=cos, b=-sin, c=sin, d=cos
        expect(result[0]).toBeCloseTo(cos, 5);      // a
        expect(result[1]).toBeCloseTo(-sin, 5);     // b
        expect(result[3]).toBeCloseTo(sin, 5);      // c
        expect(result[4]).toBeCloseTo(cos, 5);      // d
    });

    it("should compute inverse of combined transformation", () =>
    {
        // scale(2, 2) + translate(10, 20)
        const bitmapMatrix = new Float32Array([2, 0, 0, 2, 10, 20]);
    ...
    it("should return identity for singular matrix", () =>
    {
        ...
        const bitmapMatrix = new Float32Array([1, 2, 2, 4, 0, 0]); // a*d - b*c = 4 - 4 = 0
    ...
    it("should return identity for near-singular matrix", () =>
    {
        ...
        const bitmapMatrix = new Float32Array([1e-11, 0, 0, 1e-11, 0, 0]);
    ...
    {
        const bitmapMatrix = new Float32Array([1, 0, 0, 1, 0, 0]);
    ...
    {
        const bitmapMatrix = new Float32Array([-2, 0, 0, -2, 0, 0]); // scale(-2, -2)
    ...
    it("should handle skew transformation", () =>
    {
        ...
        const bitmapMatrix = new Float32Array([1, 0.5, 0.5, 1, 0, 0]);
    ...
    it("should compute correct matrix when bitmap is identity and context has transform", () =>
    {
        // When bitmap is identity, result should be identity:
        // inverse(context × identity) × context = inverse(context) × context = identity
        const bitmapMatrix = new Float32Array([1, 0, 0, 1, 0, 0]); // identity
        const contextMatrix = new Float32Array([2, 0, 0, 0, 2, 0, 50, 100, 1]); // scale(2) + translate(50, 100)

        const result = execute(bitmapMatrix, contextMatrix);
    ...
    it("should compute correct matrix for context and bitmap combination", () =>
    {
        // Test: context = scale(2), bitmap = translate(10, 20)
        // Formula: inverse(context × bitmap) × context (WebGL's transform order)
        // context × bitmap = scale(2) × translate(10, 20) = [2, 0, 0, 2, 20, 40]
        // inverse([2, 0, 0, 2, 20, 40]) = [0.5, 0, 0, 0.5, -10, -20]
        // inverse × context = [0.5, 0, 0, 0.5, -10, -20] × [2, 0, 0, 2, 0, 0] = [1, 0, 0, 1, -10, -20]
        const bitmapMatrix = new Float32Array([1, 0, 0, 1, 10, 20]); // translate(10, 20)
        const contextMatrix = new Float32Array([2, 0, 0, 0, 2, 0, 0, 0, 1]); // scale(2)

        const result = execute(bitmapMatrix, contextMatrix);

        // Expected: [1, 0, 0, 1, -10, -20] in 3x3 column-major format
        expect(result[0]).toBeCloseTo(1, 5);     // a
        expect(result[1]).toBeCloseTo(0, 5);     // b
        expect(result[3]).toBeCloseTo(0, 5);     // c
        expect(result[4]).toBeCloseTo(1, 5);     // d
        expect(result[6]).toBeCloseTo(-10, 5);   // tx
        expect(result[7]).toBeCloseTo(-20, 5);   // ty
    });
});
