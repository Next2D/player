/**
 * Experimental fixture only: NOT enabled in the renderer. See the staging
 * performance report; default scheduling increased completion latency.
 *
...
export class ExternalImageUploadBatch
{
    ...
    append (source: OffscreenCanvas, destination: GPUTexture, width: number, height: number): boolean
    {
        ...
        if (this.y + height > this.size) this.flush();
        if (!this.texture) {
    ...
    flush (): void
    {
        if (!this.encoder || !this.canvas || !this.texture) return;
