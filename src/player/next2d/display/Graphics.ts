import { GraphicsParentImpl } from "../../../interface/GraphicsParentImpl";
...
import { CanvasGradientToWebGL } from "../../../webgl/CanvasGradientToWebGL";
...
export class Graphics
{
    ...
    private _$caps: CapsStyle;
    private _$joints: JointStyle;
        ...
        if (!this._$buffer) {
            ...
            for (let idx: number = 0; idx < recode.length;) {
                ...
                switch (type) {
                    ...
                    case Graphics.GRADIENT_FILL:
                        {
                            const type: GradientType = recode[idx++];
                            const stops: ColorStopImpl[] = recode[idx++];
                            const matrix: Float32Array = recode[idx++];
                            const spread: SpreadMethod = recode[idx++];
                            const interpolation: InterpolationMethod = recode[idx++];
                    ...
                    case Graphics.GRADIENT_STROKE:
                        {
                            ...
                            const lineCap: CapsStyle = recode[idx++];
                            switch (lineCap) {
                            ...
                            const lineJoin: JointStyle = recode[idx++];
                            switch (lineJoin) {
